...
def format_tiktok(data: Dict[str, Any]) -> str:
    ...
    if download:
        body += f"\n<b>Link:</b> <a href='{download}'>Click To Download</a>"
...
async def answer_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> None:
    ...
    if result.ok:
        ...
        await send_html(update, premium_block("Unavailable", "Unable to retrieve response at this moment. Please try again later.", "⚠️"), reply_markup=premium_panel())
...
async def answer_search(update: Update, context: ContextTypes.DEFAULT_TYPE, query: str) -> None:
    ...
    async with TypingLoop(update.effective_chat.id, context.application):
        ...
        summary_prompt = f"Provide a short, smart summary. Query: {query}\nRaw: {json.dumps(raw)[:3500]}"
...
async def answer_image(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str) -> None:
    ...
    if not result.ok or not result.data or not result.data.get("bytes"):
        await send_html(update, premium_block("Image", "Failed to generate image. Try a different prompt.", "🖼"))
        return
    bio = io.BytesIO(result.data.get("bytes"))
...
async def answer_tiktok(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str) -> None:
    ...
    if not result.ok:
        await send_html(update, premium_block("TikTok Error", "Failed to fetch information from the link. API may be down.", "🎵"))
        return
...
START_TEXT = (
    ...
    f"• ff 12345678 bd\n"
    f"• just send any youtube/tiktok link directly\n\n"
    f"<b>Quick Styles:</b>\n"
    f"• style premium\n"
    f"• style concise\n"
    f"• style friendly"
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("style:"):
        ...
        if style in CHAT_STYLE_PRESETS and query.message:
            ...
            await query.message.reply_html(premium_block("Style Configuration", f"Current style: <b>{esc(style)}</b>", "🎭"), reply_markup=premium_keyboard())
        return
    if data.startswith("mode:"):
        mode = data.split(":", 1)[1].strip()
        user_modes[update.effective_user.id] = mode
        if query.message:
            await query.message.reply_html(premium_block("Mode Configuration", f"Current mode: <b>{esc(mode)}</b>", "⚙️"), reply_markup=premium_keyboard())
        return
    if data == "help:image" and query.message:
        await query.message.reply_html(premium_block("Image Generator", "Example Usage: <code>image cyberpunk city at night</code>", "🖼"), reply_markup=premium_keyboard())
        return
    if data == "help:search" and query.message:
        await query.message.reply_html(premium_block("Search Engine", "Example Usage: <code>search ai news</code>", "🔎"), reply_markup=premium_keyboard())
...
# ============================================================
# Startup / Shutdown (Modified for Render Web Service)
# ============================================================
...
async def on_startup(application: Application) -> None:
    ...
    await start_dummy_server()  # This keeps Render happy!
